...
classes = ['1I6V', '1QO1', '1KP8','4V4A','5LQW']#, 3DY4,'1KP8', '1A1S','1BXR','1VPX','1LB3']
class NewDataSet(Dataset):
    def __init__(self):
        ...
        with open(file, 'rb') as f:
            x_train = pickle.load(f, encoding='latin1')
            x_train = np.array([x_train[_]['v'] for _ in range(0, len(x_train), 1) if x_train[_]['id'] in classes if _ % 2000 < 1500])
        ...
        #gt.sort()
        #gt = np.array(gt)
        gt = np.repeat(range(len(classes)), 1500)  #ground truth labels
...
class NewDataSet_test(Dataset):
    def __init__(self):
        ...
        with open(file, 'rb') as f:
            x_train = pickle.load(f, encoding='latin1')
            x_train = np.array([x_train[_]['v'] for _ in range(0, len(x_train)) if x_train[_]['id'] in classes and _ % 2000 >= 1500])
        ...
        #gt = np.array(gt)
        # self.x_data = (x_train - np.min(x_train)) / (np.max(x_train)-np.min(x_train))
        gt = np.repeat(range(len(classes)), 500)
        self.x_data = x_train
        print(np.max(self.x_data))
        print(np.min(self.x_data))
        self.y_data = gt
