...
class DefaultModel(nn.Module):
    ...
    def _initialise_weights_(self):
        for y, m in enumerate(self.modules()):
            ...
            if isinstance(m, nn.Conv3d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.kernel_size[2]* mm.out_channels
    ...
    def _make_sobel_(self):
        ...
        '''
        sobel_filter = nn.Conv3d(1, 2, kernel_size=3, stride=1, padding=1)
        sobel_filter.weight.data[:, :, -1].copy_(
            torch.FloatTensor([[[1, 2, 1], [2, 4, 2], [1, 2, 1]]])
        )
        sobel_filter.weight.data[:, :, 0].copy_(
            torch.FloatTensor([[[0, 0, 0], [0, 0, 0], [0, 0, 0]]])
        )
        sobel_filter.weight.data[:, :, 1].copy_(
            torch.FloatTensor([[[-1, -2, -1], [-2, -4, -2], [-1, -2, -1]]])
        )
        ...
        layers = nn.Sequential(sobel_filter)
