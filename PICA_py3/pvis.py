...
def semicluster(myData,y):
    ...
    ival = myData.shape[0] / numclass # 2 = num classes
    
    iters = 5
    avg = np.empty((numclass,iters))
    for x in range(iters): 
        print('----new iter----')
        temps = np.array([myData[int(x*ival + random.randrange(0,ival))] for x in range(numclass)])
        tempLabels = np.array([x for x in range(numclass)])
...
def main():
    ...
    acc, nmi, ari = evaluate(net, test_loader, temp_loader,temppath='ftstemps.pickle')#, testpath="fts.pickle", temppath='ftstemps.pickle')
...
def evaluate(net, testloader, temploader, testpath = None, temppath = None):
    ...
    # compute accuracy
    num_classes = 5 #labels.max().item() + 1
